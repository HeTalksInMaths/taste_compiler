...
interface PersonaSim {
  persona_id: string; segment: string; job_role: string;
  market: string; content_job: string; ai_comfort: string; budget_sensitivity: string;
}
...
function chooseBestScorer(persona: PersonaSim) {
  let best = SCORER_CARDS[0]; let bestFit = 0;
...
export async function POST() {
  ...
  for (const persona of DEMO_PERSONAS) {
    ...
    const result: Record<string, unknown> = {
      persona_id: persona.persona_id, segment: persona.segment, job_role: persona.job_role,
      market: persona.market, content_job: persona.content_job, selected_scorer: scorer.title,
      scorer_id: scorer.scorer_id, fit_score: +fit.toFixed(3), reveal_probability: +prob.toFixed(3),
      will_buy: willBuy, stripe_session_id: null as string | null, stripe_session_url: null as string | null,
    };
    ...
    if (willBuy) {
      try {
        const session = await stripe.checkout.sessions.create({
          mode: 'payment',
          line_items: [{ price_data: { currency: persona.market === 'Singapore' ? 'sgd' : 'usd', product_data: { name: `${scorer.title} — Reveal Report`, description: `Persona: ${persona.persona_id} (${persona.job_role})` }, unit_amount: scorer.price_cents }, quantity: 1 }],
          metadata: { persona_id: persona.persona_id, scorer_id: scorer.scorer_id, simulation: 'nemotron_v2_1' },
          success_url: `${baseUrl}/market-dynamics/live-sim?success=true`,
          cancel_url: `${baseUrl}/market-dynamics/live-sim?cancelled=true`,
        });
  ...
  return NextResponse.json({
    ...
    personas_total: DEMO_PERSONAS.length, buyers: buyers.length,
    bounced: results.filter(r => !r.will_buy).length,
    conversion_rate: +(buyers.length / DEMO_PERSONAS.length).toFixed(3),
    total_revenue_cents: totalRevenue,
    platform_take_cents: Math.round(totalRevenue * 0.30 * 0.7),
